...
export async function GET(request) {
    ...
    const url = DJANGO_API_ENDPOINTS.ATTENDANCE_PERMISSIONS.LIST + (query ? `?${query}` : '')
...
export async function POST(request) {
    ...
    try {
        return await post_request(DJANGO_API_ENDPOINTS.ATTENDANCE_PERMISSIONS.LIST, data, access, refresh)
    } catch (e) {
        console.log('exc is -> ', e)
        console.log('beautiful error', handleError(e))
        return Response.json(handleError(e), {status: 500})
