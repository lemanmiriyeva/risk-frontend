...
export async function GET(request) {
    ...
    const url = DJANGO_API_ENDPOINTS.BULLETIN.CIRCULARS + (query ? `?${query}` : '')
...
export async function POST(request) {
    ...
    try {
        if (contentType.includes('multipart/form-data')) {
            ...
            return await post_form_request(DJANGO_API_ENDPOINTS.BULLETIN.CIRCULARS, formData, access, refresh)
        }
        const data = await request.json()
        return await post_request(DJANGO_API_ENDPOINTS.BULLETIN.CIRCULARS, data, access, refresh)
    } catch (e) {
        console.log('exc is -> ', e)
        console.log('beautiful error', handleError(e))
        return Response.json(handleError(e), {status: 500})
