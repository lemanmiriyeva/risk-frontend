...
export async function GET(request) {
    ...
    try {
        return await get_request(DJANGO_API_ENDPOINTS.CORE.MODULE_ORG_ACCESS, access, refresh)
    } catch (error) {
...
export async function POST(request) {
    ...
    try {
        return await post_request(DJANGO_API_ENDPOINTS.CORE.MODULE_ORG_ACCESS, data, access, refresh)
    } catch (error) {
        console.log('exc is -> ', error)
        console.log('beautiful error', handleError(error))
        return Response.json(handleError(error), {status: 500});
