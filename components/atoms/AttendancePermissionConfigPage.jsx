...
export default function AttendancePermissionConfigPage() {
    ...
    */
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [configResponse, usersResponse] = await Promise.all([service_api.get(API.CONFIG), service_api.get(API.USERS),]);
            ...
            setApparatus(configData.apparatus || null);


            setDepartments(Array.isArray(configData.departments) ? configData.departments : []);
            ...
            console.error("Attendance permission config error:", err);

            setError(err?.response?.data?.detail || "Konfiqurasiya məlumatlarını yükləmək mümkün olmadı.");
    ...
    const handleApparatusEnabledChange = (event) => {
        ...
        setApparatus((current) => {
            ...
            return {
                ...
                apparatus_head_enabled: enabled,
    ...
    */
    ...
    const saveApparatus = async () => {
        ...
        try {
            ...
            setApparatus(response?.data || apparatus);


            setSuccess("Aparat rəhbəri konfiqurasiyası yadda saxlanıldı.");


        } catch (err) {

            console.error("Apparatus config save error:", err);


            setError(err?.response?.data?.detail || "Aparat rəhbəri konfiqurasiyası yadda saxlanılmadı.");
    ...
    */
    ...
    const handleDepartmentManagerChange = (departmentId, enabled) => {
    ...
    */
    ...
    const handleReplacementChange = (departmentId, value) => {
        ...
        setDepartments((current) => {
            ...
            return current.map((item) => {
                ...
                const selectedUser = activeUsers.find((user) => user.id === Number(value));


                return {
                    ...item,

                    replacement_user: value === "" ? null : Number(value),

                    replacement_user_name: selectedUser?.name || null,
                };
    ...
    */
    ...
    const saveDepartment = async (department) => {
        ...
        setSavingDepartment(departmentId);
        ...
        try {
            ...
            const response = await service_api.patch(`${API.DEPARTMENT_CONFIG}${departmentId}/`, {
            ...
            setSuccess(`${department.department_name} üçün konfiqurasiya yadda saxlanıldı.`);


        } catch (err) {

            console.error("Department config save error:", err);


            setError(err?.response?.data?.detail || `"${department.department_name}" üçün konfiqurasiya yadda saxlanılmadı.`);
